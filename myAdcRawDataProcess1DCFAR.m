...
Params.freqSlope = 29.9820e12;     %chirp的斜率
...
Params.fid_rawData = fopen('adc_data_2difmove.bin','rb');
...
dataSet.radarCubeData = processingChain_rangeFFT(0);
...
% title('1d fft 结果');
% 执行多普勒FFT
dataSet.radarCubeData = processingChain_dopplerFFT(0);


%对4个接收天线Chan求和
dataSet.sumFFT2D_radarCubeData = single(zeros(Params.NChirp,Params.NSample));   %创建一个NChirp行 NSample列的空二维数组
for chanNum = 1:Params.NChan
    %把三维数组变为二维数组
    FFT2D_radarCubeData = reshape(dataSet.radarCubeData(:,chanNum,:),Params.NChirp,Params.NSample);   
    %求模->取log->相加
...
for i = 1:Params.NSample
    dopplerDim = reshape(dataSet.sumFFT2D_radarCubeData(:,i),1,Params.NChirp);  %变成一行数据
    [cfar1D_Arr,threshold] = ac_cfar1D(12,2,0.005,dopplerDim);  %进行1D cfar
...
for range = 1:Params.NSample
    ...
    objDopplerArr = [indexArr;zeros(Params.NChirp - length(indexArr),1)];   %补充长度
...
while(i<=length(objDopplerIndex))
    if(objDopplerIndex(i)==0)   % 因为数组里面有0,防止下面j取到0
        ...
        % ╮(╯▽╰)╭
        [cfar1D_Arr,threshold] = ac_cfar1D(12,2,0.03,rangeDim);  %进行1D cfar
...
function [radarCubeData] = processingChain_dopplerFFT(rangeWinType)
    ...
    switch rangeWinType
        case 1 %hann
            win = hann(Params.NChirp);
        case 2 %blackman
            win = blackman(Params.NChirp);
        otherwise
            win = rectwin(Params.NChirp);
    end
    %初始化一个三维数组存放FFT结果
    radarCubeData = single(zeros(NChirp,NChan,NRangeBin));
    for chIdx=1:Params.NChan
        for rangeIdx = 1: Params.NSample
            frameData(1,:) = dataSet.radarCubeData(:,chIdx,rangeIdx);    
            frameData = fftshift(fft(frameData .* win', NChirp));      %进行NChirp点的doppler-FFT
                                                                       %fftshift移动零频点到频谱中间---不是很懂--收藏夹
...
function [cfar_radarCubeData,tMap] = ac_cfar(NTrainRange,NGuardRange,NTrainDoppler,NGuardDoppler,PFA,inputRDM)
    global Params; 
    cfar_radarCubeData = zeros(Params.NChirp,Params.NSample);   %存放cfar后的结果,
    tMap = NaN*zeros(Params.NChirp,Params.NSample);                 %存放门限图
    %限制CUT的范围
    for i = NTrainRange+NGuardRange+1 : Params.NSample-NTrainRange-NGuardRange
        for j = NTrainDoppler+NGuardDoppler+1 : Params.NChirp-NTrainDoppler-NGuardDoppler
            %存放每次迭代的训练单元的噪音值
